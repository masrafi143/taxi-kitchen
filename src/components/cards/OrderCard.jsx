...
const OrderCard = ({order, handleOrders}) => {
  ...
  return (
    <div onClick={()=>handleOrders(order)} className="p-5 rounded-xl shadow-xl relative space-y-3">
      ...
      {/* Table Number Badge */}
      <div className="absolute right-2 top-2 w-10 h-10 flex justify-center items-center bg-amber-400 rounded-full text-white">
        {order.table_no}
      </div>
    </div>
  );
